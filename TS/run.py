...
import numpy as np
import os
os.environ['CUDA_VISIBLE_DEVICES'] = '0'  # 关键！必须放在所有 torch 操作之前
...
if __name__ == '__main__':
    ...
    def verify_cuda():
        ...
        if torch.cuda.is_available():
            ...
            print(f"GPU测试: {x.device} | 计算: {x.sum().item()}")
        else:
            print("❌ CUDA不可用")
    
    verify_cuda()  # 必须在所有模型操作前执行
    # ============================
    fix_seed = 2021
   ...
   # parser.add_argument('--use_gpu', type=int, default=1, help='use gpu (1/0)')  # 推荐
    ##-----------------------------------------------------
   # parser.add_argument('--gpu', type=int, default=0, help='gpu')
    ...
    parser.add_argument('--use_gpu', type=int, choices=[0, 1], default=1,
                       help='是否使用GPU (1:启用, 0:禁用)')
    parser.add_argument('--gpu', type=int, default=0,
                       help='使用的GPU编号（单卡时生效）')
    parser.add_argument('--gpu_type', type=str, default='cuda',
                       choices=['cuda', 'mps'], help='GPU类型 (cuda:NVIDIA, mps:Apple)')
    ...
    args.seq_len = 7680
    args.label_len = 3840 
    args.pred_len = 0 if args.task_name == 'classification' else 7680
